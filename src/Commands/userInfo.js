...
module.exports.execute = async (client,message, args) => {
  ...
  let sesli = new MessageButton()
    ...
    let pointlimit = totaldb.get(`${user.id}.pointlimit`) || 0;
      ...
      const InformationMessage = await message.channel.send({embed: infoembed.setDescription(`${user} **(\`${user.user.tag}\`)**  kullanıcısının istatistikleri;

**\`❯\` Genel Durumu**
Aktif bir şekilde **${level}**. seviyedesin, **${point}XP**'in bulunuyor ve yeni seviyeye atlamak için **${pointlimit-point}XP** kazanman lazım.`)
...
**\`❯\` Genel Durumu**
Aktif bir şekilde **${level}**. seviyedesin, **${point}XP**'in bulunuyor ve yeni seviyeye atlamak için **${pointlimit-point}XP** kazanman lazım.`), components: [row1]})
